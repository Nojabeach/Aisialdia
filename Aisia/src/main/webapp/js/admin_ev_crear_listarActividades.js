window.onload = function () {
   console.log('Script admin_ev_crear_listarActividades cargado');
    EVobtenerActividades();
  };
  
  function EVobtenerActividades() {
  ...
  function EVpintarSelect(datos) {
    ...
    EVMostrarImagen(image, select.options[0]);
  
    // Agregar evento change al select
    select.addEventListener("change", function () {
      let selectedOption = this.options[this.selectedIndex];
      EVMostrarImagen(image, selectedOption);
    });
  }
  
  function EVMostrarImagen(image, option) {
